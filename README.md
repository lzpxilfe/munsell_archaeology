...
1. `desktop\dist\MunsellSoilColor.exe` 파일을 찾습니다 (탐색기에서 위 경로로 이동)
2. 그 파일을 **마우스 오른쪽 클릭 → 바로가기 만들기**
3. 생성된 "MunsellSoilColor.exe - 바로 가기" 파일을 바탕화면으로 드래그
4. (선택) 바로가기를 다시 우클릭 → 이름 바꾸기로 "먼셀 토색 판별기" 등 원하는 이름으로 변경

exe 파일 자체를 바탕화면으로 옮겨도 되지만, 원본은 `dist` 폴더에 두고 **바로가기만**
바탕화면에 두는 걸 권장합니다 (exe가 실행 중 자기 폴더의 위치를 기준으로 동작하지는
않지만, 실수로 원본을 지우는 걸 방지할 수 있습니다).
